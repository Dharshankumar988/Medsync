...
class AIHealthMonitor:
    ...
    async def check_all(self, groq_client, model_manager) -> Dict[str, Any]:
        ...
        try:
            # 1. Check Groq
            if groq_client and groq_client.api_key and groq_client.api_key != "mock_key":
                self.health_status["components"]["groq"] = "healthy"
            else:
                self.health_status["components"]["groq"] = "degraded (fallback active)"
                
            # 2. Check HF Spaces (via model manager)
            hf_status = await model_manager.check_health()
            self.health_status["components"]["hf_spaces"] = hf_status
            
            # 3. Check RAG (stubbed for now)
            self.health_status["components"]["rag"] = "healthy (stub)"
            
            # Aggregate status
            any_degraded = any("degraded" in v or "unreachable" in v for v in self.health_status["components"].values())
